...
class MyTokenizer():
    ...
    def transform_txt(self, frase): 
        ...
        frase_punt_space = re.sub(f"[{re.escape(self.signos_punt_keep)}]", r' \g<0> ', frase) #Elimina los signos de puntuación
    ...
    def tokenize_word(self,word):
        ...
        while len(word) > 0:
            i = len(word)
            #Busca la subpalabra más larga del corpus, luego busca el prefijo más largo del corpus
            while i > 0 and word[:i] not in self.vocab_dict:
                i -= 1
           
            if i == 0: 
                #En el caso que no se encuentre devuelve el token UNK
                return ["[UNK]"]
    ...
    def encode_plus(self, raw_txt, add_special_tokens, max_length, return_attention_mask=True, return_tensors='pt',  
        ...
        result['input_ids'] = torch.tensor([self.convert_tokens_to_ids(txt_encoded)]) #TOKENS TO IDS
